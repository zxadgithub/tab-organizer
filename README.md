...
- Merge identical links: detects identical links and starts a merge flow.
- Confirm-before-merge (identical links): duplicate tabs can be reviewed before merge or keep.
- Domain grouping: keeps tabs from the same domain together.
- Stable in-domain order: within the same domain group, older tabs stay first and newly opened tabs are always appended.
- Smoother arranging: uses minimal tab moves to reduce visible jumping during auto-sort.
- Domain split window: when a domain exceeds the threshold, tabs can be moved into a dedicated window.
- Organize modes: Gentle mode keeps each window independent, while Global mode can route and split tabs across windows.
...
2. Domain routing to a dedicated window in Global mode  
3. Domain sorting in the current window  
4. Threshold-based split window in Global mode
...
- 相同链接合并：识别相同链接并触发合并流程。
- 合并前确认（相同链接）：相同链接可先进入待确认列表，由用户决定合并或保留。
- 域名排序：将相同域名标签页自动聚拢，保持更清晰的浏览顺序。
- 同域名顺序稳定：同域名内始终保持先开在前、后开在后，新开页不会插到中间。
- 排序更丝滑：采用最少移动策略，减少自动整理时的跳动感。
- 域名拆窗：当同域名标签页超过阈值时，自动归并到专属窗口。
- 整理模式：温和模式保持各窗口独立，全局模式可跨窗口归并和拆窗。
