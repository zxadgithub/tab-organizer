...
- 域名过滤：支持全量处理、排除域名、仅处理指定域名（含通配符/正则/路径规则）。
- 域名暂停处理：可按域名临时暂停自动处理若干分钟，并支持恢复。
- 管理页操作：按域名分组查看、折叠/展开、置顶域名、跳转标签页、拖拽排序、多选关闭、整组关闭、撤销关闭。
...
- Confirm-before-merge: duplicate tabs can be reviewed before merge/keep.
...
- Manager page: grouped view, collapse/expand, pin domain, focus tab, drag reorder, multi-close, group close, and undo close.
- Selection productivity: Shift range selection and Ctrl/Cmd+A for current filtered results.
- Bilingual UI: both Popup and Manager support Chinese/English with shared language preference.

### Processing Order

1. Duplicate detection (or pending review)  
2. Domain routing (to dedicated window)  
3. Domain sorting in current window  
